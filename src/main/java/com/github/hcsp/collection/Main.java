...
public class Main {
    ...
    public static Map<String, List<User>> collect(List<User> users) {
       /* Map<String, List<User>> groupByDepartment = new HashMap<>();
        for (User user : users) {
            String departmentName = user.getDepartment();
            List<User> userList = groupByDepartment.get(user.getDepartment());

            if (userList == null) {
                userList = new ArrayList<>();
                userList.add(user);
                groupByDepartment.put(departmentName, userList);
            } else {
                userList.add(user);
            }
