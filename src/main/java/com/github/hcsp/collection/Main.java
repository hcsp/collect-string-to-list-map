...
public class Main {
    ...
    public static Map<String, List<User>> collect(List<User> users) {
        Collection departmentSet = new HashSet<String>();
        Map<String, List<User>> result = new HashMap<>();

        for(User user:users){
            System.out.println(user);
            String userDepartment = user.getDepartment();
            if (departmentSet.contains(userDepartment)){
                List<User> userList = result.get(userDepartment);
                userList.add(user);
                Collections.sort(userList);
            }else{
                departmentSet.add(userDepartment);
                result.put(userDepartment,new ArrayList<User>(Arrays.asList(user)));
            }
        }
        return result;
    }
